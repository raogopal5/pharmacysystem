...
<table class="width-64 commonfont topDown">
    <tr>
        <td class="headfirst">DRUG RETURN REQUEST</td>
    </tr>
    <tr>
  ...
  <tr>
    <!--First Table-->
        <td width="49%" class="topAlign">
          <table border="0" align="left">
            ...
            <tr>
              ...
              <tr>
                               <td width="95px" class="boldDisplay topAlign">Ward/Bed</td>
                               <td class="topAlign" width="4px">:</td>
                               <td> <#if ward??> ${ward} <#else> - </#if> /  <#if bed??>${bed} <#else> - </#if></td>
                </tr>
               <tr>
              ...
              <tr>
                          ...
                          <td class="topAlign " width="2px">:</td>
                          <td><#if department??> ${department} </#if></td>
                        </tr>
               </table>
        </td>
        <!--Second Table-->
        ...
        <td  class="topAlign">
          ...
          <tr>
                          <td colspan="3" class="topAlign" style="height:28px;"></td>
                      </tr>
                 <tr>
            <td width="95px" class="boldDisplay topAlign">Return Request No</td>
            <td class="topAlign " width="2px">:</td>
            <td><#if returnRequestNumber??> ${returnRequestNumber} <#else> - </#if></td>
          </tr>
          <tr>
            <td width="95px" class="boldDisplay topAlign">Requested Date</td>
            <td class="topAlign ">:</td>
            <td><#if requestedDate??> ${requestedDate} <#else> - </#if></td>
          </tr>
          <tr>
              <td width="95px" class="boldDisplay topAlign">Return Store</td>
              <td class="topAlign ">:</td>
              <td><#if returnStore??>${returnStore} <#else> - </#if></td>
          </tr>
          <tr>
             <td width="95px" class="boldDisplay topAlign">Requested By</td>
             <td class="topAlign ">:</td>
             <td><#if requestedBy??>${requestedBy} <#else> - </#if></td>
             </tr>
             </table>
        </td>
      </tr>
...
<table cellpadding="6" class="width-100 borderapply commonfont inv-items">
    <thead>
        <tr>
            ...
            <td style="width:250px;"><span class="boldDisplay">Item Name</span></td>
            <td style="width:80px;"><span class="boldDisplay">Mfr</span></td>
            <td style="width:60px;"><span class="boldDisplay">Batch/Exp</span></td>
            <td style="width:40px;" class="headalign"><span class="boldDisplay">Rqstd. Qty</span></td>
            <td style="width:50px;" class="headalign right"><span class="boldDisplay">Unit Price</td>
            <td style="width:50px;" class="headalign right"><span class="boldDisplay">Total Amount</td>
        </tr>
    </thead>
    <tbody>
    <#assign x = 1>
		...
		<tr>
		 <td width="2px"  class="topAlign" >${x}.</td>
			...
			<td><#if dispenseItem.getMedication()??><#if dispenseItem.getMedication().getManufacturer()??>${dispenseItem.getMedication().getManufacturer()}</#if> </#if> </td>
			<td>${dispenseItem.batchNumber}/${dispenseItem.expiryDate}</td>
			<td class="text-right">${dispenseItem.requestedReturnQuantity}</td>
			<td class="text-right">${convertDecimal.decimalFormater(dispenseItem.mrp)}</td>
			<td class="text-right right">${convertDecimal.decimalFormater(dispenseItem.mrp*dispenseItem.requestedReturnQuantity)}</td>
			 <#assign x++>
		</tr>
...
<tr>
    ...
    <tr>
             <td style="text-align: left;"><span class="boldDisplay">Reason for Return:</span><#if reasonForReturn??> ${reasonForReturn}</#if></td>
                    <td></td>
        </tr>
    ...
    <tr>
            <td><span><b>Prepared By<b>:</span>&nbsp;${preparedBy} | <span class="boldDisplay"><b>Prepared On</b>:</span>&nbsp;<span>&nbsp;${preparedOn}</span> | </td>
        </tr>
        <tr>
