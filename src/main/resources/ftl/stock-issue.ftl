...
<body>
    <div class="headfirst text-center">Stock Issue</div>
    ...
    <div width="100%" class="commonfont">
        ...
        <div class="leftcolumns">
            <p><strong>Indent Priority : </strong>${indentPriority}</p>
        </div>
        <div class="rightcolumns">
            <p><strong>Indent No : </strong>${issueLines[0].sourceDocument[0].documentNumber}</p>
        </div>

        <div class="leftcolumns">
            <p><strong>Indenter : </strong>${issueLines[0].sourceDocument[0].createdBy.displayName}</p>
        </div>
    ...
    <table width="100%" cellpadding="6" class="borderapply commonfont ind-items">
        <tr>
            ...
            <td width="11%"><strong>Req.<br>Qty</strong></td>
            <td width="10%"><strong>Iss.<br>Qty</strong></td>
            <td width="10%"><strong> Batch No. </strong> </td>
            <td width="11%"><strong> Expiry Date </strong> </td>
            <td width="11%"><strong> Unit<br>MRP </strong> </td>
        </tr>
        <#list issueLines as issueLine>
        <#if issueLine.issuedQuantity.value gt 0 >
        <tr><#assign item = issueLine.item>
            <td>${issueLine?counter}</td>
        <td>${item.getName()}</br>
            <pre><b>${item.getCode()}</b></br></td>
            <td>${issueLine.sourceDocument[0].quantity.value}</td>
            <td><#if (issueLine.issuedQuantity.value??  && issueLine.issuedQuantity.value gt 0)>${issueLine.issuedQuantity.value}</#if></td>
            ...
            <td><#if issueLine.expiryDate??>${issueLine.expiryDate.format(datetimeformatter)} <#else> </#if></td>
             <td><#if issueLine.getMrp()??>${issueLine.getMrp()} <#else> </#if></td>
        </tr>
        </#if>
        </#list>
    </table>
