...
class LocationGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Location entity")
        ...
        .repeat(2) {
            ...
            .get("/pharmacy/api/locations")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new location")
            .post("/pharmacy/api/locations")
            .headers(headers_http_authenticated)
            .body(StringBody("""{"id":null, "code":"SAMPLE_TEXT", "name":"SAMPLE_TEXT", "description":"SAMPLE_TEXT", "active":null, "physicalType":"SAMPLE_TEXT", "status":"SAMPLE_TEXT", "type":"SAMPLE_TEXT", "mode":"SAMPLE_TEXT", "telecoms":"SAMPLE_TEXT", "addresses":"SAMPLE_TEXT"}""")).asJSON
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_location_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created location")
                .get("/pharmacy${new_location_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created location")
            .delete("/pharmacy${new_location_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
