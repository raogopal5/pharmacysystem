...
class OrganizationGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Organization entity")
        ...
        .repeat(2) {
            ...
            .get("/pharmacy/api/organizations")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new organization")
            .post("/pharmacy/api/organizations")
            .headers(headers_http_authenticated)
            .body(StringBody("""{"id":null, "name":"SAMPLE_TEXT", "active":null, "code":"SAMPLE_TEXT", "organizationType":"SAMPLE_TEXT", "startedOn":"2020-01-01T00:00:00.000Z", "clinical":null, "licenseNumber":"SAMPLE_TEXT", "website":"SAMPLE_TEXT"}""")).asJSON
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_organization_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created organization")
                .get("/pharmacy${new_organization_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created organization")
            .delete("/pharmacy${new_organization_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
