...
class ItemGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Item entity")
        ...
        .repeat(2) {
            ...
            .get("/pharmacy/api/items")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new item")
            .post("/pharmacy/api/items")
            .headers(headers_http_authenticated)
            .body(StringBody("""{"id":null, "code":"SAMPLE_TEXT", "name":"SAMPLE_TEXT", "description":"SAMPLE_TEXT", "batchTracked":null, "expiryDateRequired":null, "active":null, "remarks":"SAMPLE_TEXT", "fsnType":null, "vedCategory":null, "type":"SAMPLE_TEXT", "group":"SAMPLE_TEXT"}""")).asJSON
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_item_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created item")
                .get("/pharmacy${new_item_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created item")
            .delete("/pharmacy${new_item_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
