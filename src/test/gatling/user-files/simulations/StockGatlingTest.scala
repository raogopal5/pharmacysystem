...
class StockGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Stock entity")
        ...
        .repeat(2) {
            ...
            .get("/pharmacy/api/stocks")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new stock")
            .post("/pharmacy/api/stocks")
            .headers(headers_http_authenticated)
            .body(StringBody("""{"id":null, "itemId":null, "batchNo":"SAMPLE_TEXT", "expiryDate":"2020-01-01T00:00:00.000Z", "owner":"SAMPLE_TEXT", "cost":null, "mrp":null, "quantity":null, "stockValue":null, "storeId":null, "locatorId":null, "supplier":"SAMPLE_TEXT", "uomId":null, "sku":"SAMPLE_TEXT", "unitId":null, "consignment":null}""")).asJSON
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_stock_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created stock")
                .get("/pharmacy${new_stock_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created stock")
            .delete("/pharmacy${new_stock_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
