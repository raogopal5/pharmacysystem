...
import org.nh.pharmacy.config.KafkaProperties;
...
class PharmacyKafkaResourceIT {
    ...
    void setup() {
        ...
        PharmacyKafkaResource kafkaResource = new PharmacyKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/pharmacy-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/pharmacy-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
